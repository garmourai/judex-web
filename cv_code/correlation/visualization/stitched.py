...
def load_correlated_pairs_from_tracker_csvs_realtime(
    ...
    Args:
        output_dir: Correlation output directory containing global tracker.csv
    ...
    try:
        ...
        print(f"[CorrelationWorker]    ⚠️  Error reading tracker CSV {tracker_path}: {e}")

    return result


def load_selected_pair_costs_from_match_decisions(
    output_dir: str,
    frame_segments: List[Tuple[int, int]],
) -> Dict[int, List[float]]:
    """
    Load selected pair formula1 costs from match_decisions.csv for each frame.
    Costs are ordered by (src_index, sink_index) to produce stable A/B/C... labels.
    """
    selected_costs_by_frame: Dict[int, List[Tuple[int, int, float]]] = {}
    decisions_path = os.path.join(output_dir, "match_decisions.csv")
    if not os.path.isfile(decisions_path):
        return {}
    frame_ranges = frame_segments or []
    try:
        with open(decisions_path, "r", newline="") as f:
            ...
            for row in reader:
                try:
                    ...
                    if frame_ranges and not any(s <= frame_num <= e for s, e in frame_ranges):
                        continue
                    src_idx = int(row["src_index"])
                    sink_idx = int(row["sink_index"])
                    cost = float(row.get("cost_formula1", "nan"))
                except (ValueError, TypeError, KeyError):
                    continue
                if frame_num not in selected_costs_by_frame:
                    selected_costs_by_frame[frame_num] = []
                selected_costs_by_frame[frame_num].append((src_idx, sink_idx, cost))
    except Exception as e:
        print(
            f"[CorrelationWorker]    ⚠️  Error reading match decision CSV {decisions_path}: {e}"
        )

    result: Dict[int, List[float]] = {}
    for frame_num, rows in selected_costs_by_frame.items():
        rows_sorted = sorted(rows, key=lambda x: (x[0], x[1]))
        result[frame_num] = [cost for _, _, cost in rows_sorted]
    return result
...
def _write_stitched_frame(
    ...
    pair_costs_per_frame: Dict[int, List[float]] = None,
) -> None:
    ...
    costs = pair_costs_per_frame.get(fnwd, []) if pair_costs_per_frame is not None else []
    _draw_circles_with_labels(img1, pts_cam1, labels, costs=costs, color_bgr=(0, 255, 0))
    _draw_circles_with_labels(img2, pts_cam2, labels, costs=costs, color_bgr=(0, 255, 0))
        ...
        try:
            for fnwd in frame_numbers:
                ...
                _write_stitched_frame(
                    ...
                    pair_costs_per_frame=pair_costs_per_frame,
                )
                segment_frames += 1
        finally:
            writer.release()
