...
This module provides trajectory detection functionality for the realtime flow,
separated from the original shot_detection_and_trajectory_analysis module.
"""
import numpy as np
from .detection import *
from .trajectory_context import (
...
class DetectionTree:
    ...
    def check_null(self, coordinates):
        """Check if coordinates are null (0,0,0)."""
    ...
    def min_distance_per_frame(self, detections, coordinates, frame_number):
        """Calculate minimum distance per frame for trajectory validation."""
    ...
    def get_latest_frame_in_queue(self, queue):
        """Get the latest frame number in a queue."""
        return max(det[1] for det in queue.detections)

    def perpendicular_distance_to_ray(self, p1, p2, p3, point):
        """Calculate perpendicular distance from point to ray defined by p1->p2->p3."""
        a = np.array(p1)
        b = np.array(p3)
    ...
    def mean_distance_among_last_three(self, queue):
        """Calculate mean distance among last three detections in queue."""
    ...
    def enqueue_detections(self, points, frame_number):
        """Assign detections to existing queues or create new ones."""
        assigned = set()
        ...
        for i, coordinates in enumerate(points):
            ...
            for queue in sorted_queues:
                ...
                if dist < VERY_CLOSE_THRESHOLD:
                    candidate_queues.append((queue, dist, self.get_latest_frame_in_queue(queue), 'very_close'))
                    ...
                    if perp_dist is not None and perp_dist < PERPENDICULAR_THRESHOLD:
                        candidate_queues.append((queue, perp_dist, self.get_latest_frame_in_queue(queue), 'line_fit'))
                    ...
                    if self.euclidean_distance(last_coords, coordinates) < FAR_DISTANCE_MULTIPLIER * mean_dist:
                        candidate_queues.append((queue, dist, self.get_latest_frame_in_queue(queue), 'far_match'))
            ...
            if close_queues:
                selected_queue = max(
                    close_queues,
                    key=lambda q: (len(q.detections), max(d[1] for d in q.detections))
                )
            else:
                selected_queue = None
        ...
        for i, coordinates in enumerate(points):
            if i not in assigned:
                ...
                if queue.trajectory_index is None:
                    ...
                    queue.push((coordinates, frame_number), self.trajectories[queue.trajectory_index])
                self.queues.append(queue)

    def process_queues(self, frame_number):
        """Process queues and move expired ones to trajectories."""
        ...
        for idx, queue in enumerate(self.queues):
            if idx not in queue_idxs_to_remove:
                persisting_queues.append(queue)
        self.queues = persisting_queues
    ...
    def prune_trajectory(self, frame_number):
        for key, value in self.trajectories.items():
            ...
            if var_trajectory is not None:
                ...
                if frame_number - var_trajectory_latest_frame > FRAME_DIFF_THRESHOLD \
                or frame_number == self.end_frame:
    ...
    def add_detections(self, points, frame_number):
        """Main method to add detections to the tree."""
        if len(points) == 0 or (len(points) == 1 and self.check_null(points[0])):
            pass
        else:
    ...
    def get_handoff_context(self, frame_gap_threshold: int = 10) -> TrajectoryHandoffContext:
        """
        Extract handoff context for continuing trajectories in next segment.
        
        This captures:
        - Active queues (trajectory hypotheses still being built)
        - Active trajectories (not yet stored, might continue)
        - State counters for continuity
        
        Args:
            frame_gap_threshold: Trajectories with gap larger than this are considered finished
        
        Returns:
            TrajectoryHandoffContext with all state needed for next segment
        """
                ...
                if frame_gap <= frame_gap_threshold:
                    ...
                    for frame_num, det in traj.detections.items():
                        detection_states[frame_num] = DetectionState(
                            ...
                            in_motion=det.in_motion,
                            can_pick=det.can_pick
                        )
    ...
    def restore_from_context(self, context: TrajectoryHandoffContext):
        """
        Restore tree state from previous segment's handoff context.
        
        This allows continuing trajectory building from where the previous
        segment left off.
        
        Args:
            context: TrajectoryHandoffContext from previous segment
        """
        ...
        for traj_id, traj_state in context.active_trajectories.items():
            ...
            for frame_num, det_state in traj_state.detections.items():
                det = Detection(
                    ...
                    y=det_state.y,
                    z=det_state.z,
                    frame_number=det_state.frame_number,
                    in_motion=det_state.in_motion,
                    can_pick=det_state.can_pick
                )
        ...
        print(f"  📥 Restored context: {len(context.queues)} queues, "
              f"{len(context.active_trajectories)} active trajectories, "
              f"trajectory_count={self.trajectory_count}", flush=True)
    
    def finalize_for_handoff(self, current_frame: int, frame_gap_threshold: int = 10):
        """
        Finalize trajectories for handoff - store truly finished ones,
        keep active ones for context.
        
        Unlike prune_trajectory() which stores everything at segment end,
        this only stores trajectories that are definitely finished.
        
        Args:
            current_frame: Current frame number
            frame_gap_threshold: Trajectories with gap larger than this are finished
        
        Returns:
            List of trajectories that were stored (finished)
        """
        newly_stored = []
